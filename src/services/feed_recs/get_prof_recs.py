...
def get_creators(all_listeners, all_users):
    ...
    for creator in creators:
        ...
        if len(followers_data) == 0:
            # 30 inital preferences + 1 age variable
...
def get_model():
    """
    Returns the previously pickled model
    """
...
def get_creator_recs(creator_ids, final_dataset_X):   
    ...
    result = np.array(list(zip(np.array(creator_ids), model.predict_proba(final_dataset_X)[:, PROB_INDEX])), dtype=dtype)
    ...
    creator_recs = list(db.users.find({"_id": {"$in": list(set(result))}}, {"avatar": 1, "username": 1, "bio": 1, "firstName": 1, "lastName": 1}))
